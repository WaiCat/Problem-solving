...
{
	if (s == e) return seg[node] = { arr[s], s }; // start �� end �� ��ġ�� ��ġ�ϸ� input[start] ���� �־��ش�.
	int mid = (s + e) / 2;
	return seg[node] = min(init(2 * node, s, mid, seg, arr), init(2 * node + 1, mid + 1, e, seg, arr)); // �ٸ� ������ ������ 
}

pair<int, int> query(int node, int s, int e, int l, int r, pair<int, int>* seg)
{
	if (e < l || r < s) return { INF, INF }; // ã�ƾ��ϴ� ������ ��屸���� ��ġ�� ���� ��
	if (l <= s && e <= r) return seg[node]; // ã�ƾ��ϴ� �������� ��屸���� ���Ե� ��
	int mid = (s + e) / 2;
	// ã�ƾ��ϴ� ������ ��屸���� ���Եǰų�, �κ������� ��ġ�� ���
	return min(query(2 * node, s, mid, l, r, seg), query(2 * node + 1, mid + 1, e, l, r, seg));
}

void update(int node, int s, int e, int idx, int val, pair<int, int>* seg, int* arr) {
	if (s == e && s == idx) { // leaf ��忡 �������� ��
...
int main()
{
	...
	while (true) {
		...
		if (command == "q") {
			...
			//cout << command << " " << a << " " << b << " " << result.second << endl;
			//answer += findIndex(arr, n, result);
		}
		if (command == "c") {
			arr[a] = b;
			// ���׸�Ʈ Ʈ��(seg �迭) ������Ʈ
